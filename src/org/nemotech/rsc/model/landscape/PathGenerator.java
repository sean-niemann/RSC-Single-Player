...
public class PathGenerator {
    ...
    public static final int[][] ALLOWED_WALL_ID_TYPES = { 
        { 5, 6, 14 }, // VERTICAL WALL ID's (42) = broken down wall
        { 5, 6, 14 }, // HORIZONTAL WALL ID's (42)
        { 229, 5 } //{ 229, 5 } // DIAGONAL WALL ID's 
    }; 
        ...
        while (true) {
            ...
            if (newX == -1) {
                if (ourX > enemyX && ourY > enemyY) {
                    return !isWallAllowed(stuckX + 1, stuckY + 1); // Northeast Direction
                }
                else if (ourX > enemyX && ourY <= enemyY) {
                    return !isWallAllowed(stuckX + 1, stuckY); // Northwest Direction
                }
                else if (ourX <= enemyX && ourY > enemyY) {
                    return !isWallAllowed(stuckX, stuckY + 1); // Southeast Direction
                }
                else {
