...
public class Terrain {
    ...
    public static final int COLOR_TRANSPARENT = 12345678;
    
    private ZipFile tileArchive;
    
    protected int[] selectedX, selectedY;
    protected int[] groundTextures;
    protected int[][] objectDirs, walkableValue, pathFindSource;
    protected boolean playerIsAlive;
    
    protected Model parentModel;
    protected Model[] terrainModels;
    protected Model[][] wallModels;
    protected Model[][] roofModels;
    
    private Surface gameImage;
    ...
    public Terrain(Scene camera, Surface gameImage) {
        ...
        wallModels = new Model[4][64];
        roofModels = new Model[4][64];
        pathFindSource = new int[96][96];
        requiresClean = true;
        terrainModels = new Model[64];
        groundTextures = new int[256];
        ...
        for (int i = 0; i < 64; i++) {
            groundTextures[i] = Scene.rgb(255 - i * 4, 255 - (int) ((double) i * 1.75D), 255 - i * 4);
        }
        for (int j = 0; j < 64; j++) {
            groundTextures[j + 64] = Scene.rgb(j * 3, 144, 0);
        }
        for (int k = 0; k < 64; k++) {
            groundTextures[k + 128] = Scene.rgb(192 - (int) ((double) k * 1.5D), 144 - (int) ((double) k * 1.5D), 0);
        }
        for (int l = 0; l < 64; l++) {
            groundTextures[l + 192] = Scene.rgb(96 - (int) ((double) l * 1.5D), 48 + (int) ((double) l * 1.5D), 0);
        }
    }
    ...
    public void updateObject(int x, int y, int k, int l) {
        ...
        if (EntityManager.getGameObjectDef(k).getType() == 1 || EntityManager.getGameObjectDef(k).getType() == 2) {
            int i1;
            int j1;
            if (l == 0 || l == 4) {
                i1 = EntityManager.getGameObjectDef(k).getWidth();
                j1 = EntityManager.getGameObjectDef(k).getHeight();
            }
            else {
                j1 = EntityManager.getGameObjectDef(k).getWidth();
                i1 = EntityManager.getGameObjectDef(k).getHeight();
            }
            for (int k1 = x; k1 < x + i1; k1++) {
                for (int l1 = y; l1 < y + j1; l1++) {
                    if (EntityManager.getGameObjectDef(k).getType() == 1)
        ...
        for (int i = 0; i < 96; i++) {
            for (int j = 0; j < 96; j++) {
                if (getGroundTexturesOverlay(i, j) == 250) {
                    ...
                    else if (j == 47 && getGroundTexturesOverlay(i, j + 1) != 250 && getGroundTexturesOverlay(i, j + 1) != 2) {
                        setGroundTexturesOverlay(i, j, 9);
                    }
                    else {
    ...
    public void loadSector(int x, int y, int plane) {
        reset();
        int l = (x + 24) / 48;
        int i1 = (y + 24) / 48;
    ...
    public void method402(int i, int j, int k, int l, int i1) {
        Model model = terrainModels[i + j * 8];
    ...
    public void method403(int i, int j, int k, int l, int i1) {
        int j1 = EntityManager.getDoor(i).getHeight();
        if (pathFindSource[j][k] < 0x13880) {
            pathFindSource[j][k] += 0x13880 + j1;
        }
        if (pathFindSource[l][i1] < 0x13880) {
            pathFindSource[l][i1] += 0x13880 + j1;
        }
    }
    ...
    public void method408(int i, int j, int k, int l) {
        if (i < 0 || j < 0 || i >= 95 || j >= 95)
            return;
        if (EntityManager.getDoor(l).getType() == 1) {
            if (k == 0) {
                walkableValue[i][j] |= 1;
                if (j > 0) {
                    orWalkable(i, j - 1, 4);
    ...
    public void loadSection(int i, int j, int k, boolean flag) {
        int l = (i + 24) / 48;
        int i1 = (j + 24) / 48;
        Terrain.this.loadSection(l - 1, i1 - 1, k, 0);
        Terrain.this.loadSection(l, i1 - 1, k, 1);
        Terrain.this.loadSection(l - 1, i1, k, 2);
        Terrain.this.loadSection(l, i1, k, 3);
        setTiles();
        if (parentModel == null) {
            parentModel = new Model(18688, 18688, true, true, false, false, true);
        }
        if (flag) {
            ...
            for (int j2 = 0; j2 < 96; j2++) {
                for (int i3 = 0; i3 < 96; i3++) {
                    ...
                    if (getGroundTexturesOverlay(j2, i3) > 0 && EntityManager.getTile(getGroundTexturesOverlay(j2, i3) - 1).getUnknown() == 4)
                        i4 = 0;
                    if (getGroundTexturesOverlay(j2 - 1, i3) > 0 && EntityManager.getTile(getGroundTexturesOverlay(j2 - 1, i3) - 1).getUnknown() == 4)
                        i4 = 0;
                    if (getGroundTexturesOverlay(j2, i3 - 1) > 0 && EntityManager.getTile(getGroundTexturesOverlay(j2, i3 - 1) - 1).getUnknown() == 4)
                        i4 = 0;
                    if (getGroundTexturesOverlay(j2 - 1, i3 - 1) > 0 && EntityManager.getTile(getGroundTexturesOverlay(j2 - 1, i3 - 1) - 1).getUnknown() == 4)
                        i4 = 0;
            ...
            for (int j3 = 0; j3 < 95; j3++) {
                for (int j4 = 0; j4 < 95; j4++) {
                    ...
                    if (getGroundTexturesOverlay(j3, j4) > 0) {
                        ...
                        int l5 = EntityManager.getTile(l16 - 1).getUnknown();
                        int i19 = method427(j3, j4);
                        k7 = i10 = EntityManager.getTile(l16 - 1).getColour();
                        if (l5 == 4) {
                            k7 = 1;
                            i10 = 1;
                            if (l16 == 12) {
                        ...
                        if (l5 == 5) {
                            if (getDiagonalWalls(j3, j4) > 0 && getDiagonalWalls(j3, j4) < 24000)
                                ...
                                else if (getOverlayIfRequired(j3 + 1, j4, k12) != 0xbc614e && getOverlayIfRequired(j3, j4 + 1, k12) != 0xbc614e) {
                        ...
                        if (EntityManager.getTile(l16 - 1).getObjectType() != 0)
                            walkableValue[j3][j4] |= 0x40;
                        if (EntityManager.getTile(l16 - 1).getUnknown() == 2)
                            walkableValue[j3][j4] |= 0x80;
                    }
                    method413(j3, j4, l14, k7, i10);
                    int i17 = ((getGroundElevation(j3 + 1, j4 + 1) - getGroundElevation(j3 + 1, j4)) + getGroundElevation(j3, j4 + 1)) - getGroundElevation(j3, j4);
                    if (k7 != i10 || i17 != 0) {
                        int ai[] = new int[3];
                        int ai7[] = new int[3];
                        if (l14 == 0) {
            ...
            for (int k4 = 1; k4 < 95; k4++) {
                for (int i6 = 1; i6 < 95; i6++) {
                    if (getGroundTexturesOverlay(k4, i6) > 0 && EntityManager.getTile(getGroundTexturesOverlay(k4, i6) - 1).getUnknown() == 4) {
                        int l7 = EntityManager.getTile(getGroundTexturesOverlay(k4, i6) - 1).getColour();
                        int j10 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6), i6 * 128);
                        int l12 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6), i6 * 128);
                        int i15 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6 + 1), (i6 + 1) * 128);
                        int j17 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6 + 1), (i6 + 1) * 128);
                        int ai2[] = {
                                j10, l12, i15, j17
                        };
                    ...
                    else if (getGroundTexturesOverlay(k4, i6) == 0 || EntityManager.getTile(getGroundTexturesOverlay(k4, i6) - 1).getUnknown() != 3) {
                        if (getGroundTexturesOverlay(k4, i6 + 1) > 0 && EntityManager.getTile(getGroundTexturesOverlay(k4, i6 + 1) - 1).getUnknown() == 4) {
                            int i8 = EntityManager.getTile(getGroundTexturesOverlay(k4, i6 + 1) - 1).getColour();
                            int k10 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6), i6 * 128);
                            int i13 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6), i6 * 128);
                            int j15 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6 + 1), (i6 + 1) * 128);
                            int k17 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6 + 1), (i6 + 1) * 128);
                            int ai3[] = {
                                    k10, i13, j15, k17
                            };
                        ...
                        if (getGroundTexturesOverlay(k4, i6 - 1) > 0 && EntityManager.getTile(getGroundTexturesOverlay(k4, i6 - 1) - 1).getUnknown() == 4) {
                            int j8 = EntityManager.getTile(getGroundTexturesOverlay(k4, i6 - 1) - 1).getColour();
                            int l10 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6), i6 * 128);
                            int j13 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6), i6 * 128);
                            int k15 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6 + 1), (i6 + 1) * 128);
                            int l17 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6 + 1), (i6 + 1) * 128);
                            int ai4[] = {
                                    l10, j13, k15, l17
                            };
                        ...
                        if (getGroundTexturesOverlay(k4 + 1, i6) > 0 && EntityManager.getTile(getGroundTexturesOverlay(k4 + 1, i6) - 1).getUnknown() == 4) {
                            int k8 = EntityManager.getTile(getGroundTexturesOverlay(k4 + 1, i6) - 1).getColour();
                            int i11 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6), i6 * 128);
                            int k13 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6), i6 * 128);
                            int l15 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6 + 1), (i6 + 1) * 128);
                            int i18 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6 + 1), (i6 + 1) * 128);
                            int ai5[] = {
                                    i11, k13, l15, i18
                            };
                        ...
                        if (getGroundTexturesOverlay(k4 - 1, i6) > 0 && EntityManager.getTile(getGroundTexturesOverlay(k4 - 1, i6) - 1).getUnknown() == 4) {
                            int l8 = EntityManager.getTile(getGroundTexturesOverlay(k4 - 1, i6) - 1).getColour();
                            int j11 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6), i6 * 128);
                            int l13 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6), i6 * 128);
                            int i16 = model.vertexAt((k4 + 1) * 128, -getGroundElevation(k4 + 1, i6 + 1), (i6 + 1) * 128);
                            int j18 = model.vertexAt(k4 * 128, -getGroundElevation(k4, i6 + 1), (i6 + 1) * 128);
                            int ai6[] = {
                                    j11, l13, i16, j18
                            };
            ...
            for (int j6 = 0; j6 < 64; j6++) {
                camera.addModel(terrainModels[j6]);
        }
            for (int i9 = 0; i9 < 96; i9++) {
                for (int k11 = 0; k11 < 96; k11++) {
                    pathFindSource[i9][k11] = getGroundElevation(i9, k11);
                }
            }
        }
        parentModel.clear();
        ...
        for (int i2 = 0; i2 < 95; i2++) {
            for (int k2 = 0; k2 < 95; k2++) {
                ...
                if (k3 > 0 && EntityManager.getDoor(k3 - 1).getVisibility() == 0) {
                    method421(parentModel, k3 - 1, i2, k2, i2 + 1, k2);
                    if (flag && EntityManager.getDoor(k3 - 1).getType() != 0) {
                    ...
                    if (flag) {
                        gameImage.drawLineHoriz(i2 * 3, k2 * 3, 3, k1);
                    }
                }
                k3 = getHorizontalWall(i2, k2);
                if (k3 > 0 && EntityManager.getDoor(k3 - 1).getVisibility() == 0) {
                    method421(parentModel, k3 - 1, i2, k2, i2, k2 + 1);
                    if (flag && EntityManager.getDoor(k3 - 1).getType() != 0) {
                    ...
                    if (flag) {
                        gameImage.drawLineVert(i2 * 3, k2 * 3, 3, k1);
                    }
                }
                k3 = getDiagonalWalls(i2, k2);
                if (k3 > 0 && k3 < 12000 && EntityManager.getDoor(k3 - 1).getVisibility() == 0) {
                    method421(parentModel, k3 - 1, i2, k2, i2 + 1, k2 + 1);
                    if (flag && EntityManager.getDoor(k3 - 1).getType() != 0) {
                        walkableValue[i2][k2] |= 0x20;
                    }
                    if (flag) {
                        gameImage.setPixel(i2 * 3, k2 * 3, k1);
                        gameImage.setPixel(i2 * 3 + 1, k2 * 3 + 1, k1);
                        gameImage.setPixel(i2 * 3 + 2, k2 * 3 + 2, k1);
                    }
                }
                if (k3 > 12000 && k3 < 24000 && EntityManager.getDoor(k3 - 12001).getVisibility() == 0) {
                    method421(parentModel, k3 - 12001, i2 + 1, k2, i2, k2 + 1);
                    if (flag && EntityManager.getDoor(k3 - 12001).getType() != 0) {
                        walkableValue[i2][k2] |= 0x10;
                    }
                    if (flag) {
                        gameImage.setPixel(i2 * 3 + 2, k2 * 3, k1);
                        gameImage.setPixel(i2 * 3 + 1, k2 * 3 + 1, k1);
                        gameImage.setPixel(i2 * 3, k2 * 3 + 2, k1);
                    }
                }
            }
        }
        ...
        if (flag) {
            gameImage.drawSpriteMinimap(mudclient.getInstance().spriteMedia - 1, 0, 0, 285, 285);
        }
        parentModel.setLight(false, 60, 24, -50, -10, -50);
        wallModels[k] = parentModel.split(0, 0, 1536, 1536, 8, 64, 338, true);
        for (int l2 = 0; l2 < 64; l2++) {
            camera.addModel(wallModels[k][l2]);
    }
        ...
        for (int i5 = 1; i5 < 95; i5++) {
            for (int l6 = 1; l6 < 95; l6++) {
                ...
                if (j9 > 0) {
                    ...
                    int j23 = l6 + 1;
                    int l23 = 0;
                    int j24 = pathFindSource[l11][i14];
                    int l24 = pathFindSource[j16][k18];
                    int j25 = pathFindSource[j19][j21];
                    int l25 = pathFindSource[l22][j23];
                    if (j24 > 0x13880)
                        j24 -= 0x13880;
                    if (l24 > 0x13880)
                        l24 -= 0x13880;
                    ...
                    if (l23 >= 0x13880)
                        l23 -= 0x13880;
                    if (j24 < 0x13880)
                        pathFindSource[l11][i14] = l23;
                    else
                        pathFindSource[l11][i14] -= 0x13880;
                    if (l24 < 0x13880)
                        pathFindSource[j16][k18] = l23;
                    else
                        pathFindSource[j16][k18] -= 0x13880;
                    if (j25 < 0x13880)
                        pathFindSource[j19][j21] = l23;
                    else
                        pathFindSource[j19][j21] -= 0x13880;
                    if (l25 < 0x13880)
                        pathFindSource[l22][j23] = l23;
                    else
                        pathFindSource[l22][j23] -= 0x13880;
                }
            }
        }
        ...
        for (int i7 = 1; i7 < 95; i7++) {
            for (int k9 = 1; k9 < 95; k9++) {
                ...
                if (i12 > 0) {
                    ...
                    int l26 = k25;
                    int i27 = i26;
                    int j27 = pathFindSource[j14][k16];
                    int k27 = pathFindSource[l18][k19];
                    int l27 = pathFindSource[k21][i23];
                    int i28 = pathFindSource[k23][i24];
                    int j28 = EntityManager.getElevation(i12 - 1).getUnknown1();
                    if (method424(j14, k16) && j27 < 0x13880) {
                        j27 += j28 + 0x13880;
                        pathFindSource[j14][k16] = j27;
                    }
                    if (method424(l18, k19) && k27 < 0x13880) {
                        k27 += j28 + 0x13880;
                        pathFindSource[l18][k19] = k27;
                    }
                    if (method424(k21, i23) && l27 < 0x13880) {
                        l27 += j28 + 0x13880;
                        pathFindSource[k21][i23] = l27;
                    }
                    if (method424(k23, i24) && i28 < 0x13880) {
                        i28 += j28 + 0x13880;
                        pathFindSource[k23][i24] = i28;
                    }
                    if (j27 >= 0x13880)
                        j27 -= 0x13880;
                    if (k27 >= 0x13880)
                    ...
                    if (!method416(k23, i24 + 1))
                        i27 += byte0;
                    i12 = EntityManager.getElevation(i12 - 1).getUnknown2();
                    j27 = -j27;
                    k27 = -k27;
                    l27 = -l27;
                    i28 = -i28;
                    ...
                    else if (getDiagonalWalls(i7, k9) > 12000 && getDiagonalWalls(i7, k9) < 24000 && getRoofTexture(i7 + 1, k9 + 1) == 0) {
                        int ai9[] = new int[3];
                    ...
                    else if (getDiagonalWalls(i7, k9) > 0 && getDiagonalWalls(i7, k9) < 12000 && getRoofTexture(i7 + 1, k9 - 1) == 0) {
                        int ai10[] = new int[3];
        ...
        roofModels[k] = parentModel.split(0, 0, 1536, 1536, 8, 64, 169, true);
        for (int l9 = 0; l9 < 64; l9++) {
            camera.addModel(roofModels[k][l9]);
    }
        ...
        for (int j12 = 0; j12 < 96; j12++) {
            for (int k14 = 0; k14 < 96; k14++) {
                if (pathFindSource[j12][k14] >= 0x13880) {
                    pathFindSource[j12][k14] -= 0x13880;
                }
            }
        }
    }
    ...
    public void method413(int i, int j, int k, int l, int i1) {
        ...
        i2 = i2 >> 1 & 0x7f7f7f;
        if (k == 0) {
            gameImage.drawLineHoriz(j1, k1, 3, l1);
            gameImage.drawLineHoriz(j1, k1 + 1, 2, l1);
            gameImage.drawLineHoriz(j1, k1 + 2, 1, l1);
            gameImage.drawLineHoriz(j1 + 2, k1 + 1, 1, i2);
            gameImage.drawLineHoriz(j1 + 1, k1 + 2, 2, i2);
        }
        else if (k == 1) {
            gameImage.drawLineHoriz(j1, k1, 3, i2);
            gameImage.drawLineHoriz(j1 + 1, k1 + 1, 2, i2);
            gameImage.drawLineHoriz(j1 + 2, k1 + 2, 1, i2);
            gameImage.drawLineHoriz(j1, k1 + 1, 1, l1);
            gameImage.drawLineHoriz(j1, k1 + 2, 2, l1);
        }
    }
    ...
    public void updateDoor(int x, int y, int dir, int type) {
        ...
        if (EntityManager.getDoor(type).getType() == 1) {
            switch (dir) {
                case 0:
    ...
    public void method421(Model model, int i, int j, int k, int l, int i1) {
        ...
        method419(l, i1, 40);
        int j1 = EntityManager.getDoor(i).getHeight();
        int k1 = EntityManager.getDoor(i).getTextureFront();
        int l1 = EntityManager.getDoor(i).getTextureBack();
        int i2 = j * 128;
        int j2 = k * 128;
        int k2 = l * 128;
        int l2 = i1 * 128;
        int i3 = model.vertexAt(i2, -pathFindSource[j][k], j2);
        int j3 = model.vertexAt(i2, -pathFindSource[j][k] - j1, j2);
        int k3 = model.vertexAt(k2, -pathFindSource[l][i1] - j1, l2);
        int l3 = model.vertexAt(k2, -pathFindSource[l][i1], l2);
        int i4 = model.createFace(4, new int[]{i3, j3, k3, l3}, k1, l1);
        if (EntityManager.getDoor(i).getVisibility() == 5) {
    ...
    public int getOverlayIfRequired(int x, int y, int underlay) {
        ...
        return EntityManager.getTile(texture - 1).getColour();
    }
    ...
    public int method427(int x, int y) {
        ...
        return EntityManager.getTile(texture - 1).getUnknown() != 2 ? 0 : 1;
    }

    public void loadSectorObjects(Model[] models) {
        for (int i = 0; i < 94; i++) { // x
            for (int j = 0; j < 94; j++) { // y
                if (getDiagonalWalls(i, j) > 48000 && getDiagonalWalls(i, j) < 60000) {
                    int k = getDiagonalWalls(i, j) - 48001; // id
                    /* begin rotating objects for transistion screen */
                    registerObjectDir(83, 66, 4); // general store sign
                    registerObjectDir(73, 92, 4); // bob's axes sign
                    registerObjectDir(85, 79, 4); // castle chair 1
                    registerObjectDir(86, 79, 4); // castle chair 2
                    registerObjectDir(87, 79, 4); // castle chair 3
                    registerObjectDir(88, 79, 4); // castle chair 4
                    registerObjectDir(64, 40, 4); // wooden gate
                    registerObjectDir(80, 82, 4); // castle double doors
                    registerObjectDir(69, 74, 1); // well
                    /* end */
                    ...
                    method412(i, j, k, l);
                    Model model = models[EntityManager.getGameObjectDef(k).modelID].copy(false, true, false, false);
                    int k1 = ((i + i + i1) * 128) / 2;
                    int i2 = ((j + j + j1) * 128) / 2;
                    model.translate(k1, -getAveragedElevation(k1, i2), i2);
    ...
    private void registerObjectDir(int x, int y, int dir) {
