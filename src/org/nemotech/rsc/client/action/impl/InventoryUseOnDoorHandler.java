...
public class InventoryUseOnDoorHandler implements ActionHandler {

    public void handleInventoryUseOnDoor(int item, int x, int y) {
        ...
        if (object == null || object.getType() == 0 || invItem == null) {
            player.setSuspiciousPlayer(true);
            return;
        }
        handleDoor(player, object, invItem);
        //tile.cleanItself();
    }

    private void handleDoor(final Player player, final GameObject object, final InvItem item) {
        ...
        World.getWorld().getDelayedEventHandler().add(new WalkToObjectEvent(player, object, false) {
            public void arrived() {
                ...
                if (owner.isBusy() || owner.isRanging() || !owner.getInventory().contains(item) || !owner.withinRange(object, 3) || owner.getStatus() != Action.USING_INVITEM_ON_DOOR) {
                    return;
                }
                owner.reset();
                ...
                if (PluginManager.getInstance().blockDefaultAction("InvUseOnWallObject", new Object[]{ object, item, owner })) {
                    return;
                }
            }
        });
