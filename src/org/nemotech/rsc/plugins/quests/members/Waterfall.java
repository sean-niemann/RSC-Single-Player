...
// 'old key' on door leading to end of area does not work!
public class Waterfall extends Plugin implements QuestInterface,TalkToNpcListener,
    ...
    public void handleReward(Player p) {
        ...
        addItem(p, 172, 2);
        addItem(p, 161, 2);
        p.incQuestExp(0, p.getMaxStat(0) * 225 + 250);
        p.incQuestExp(2, p.getMaxStat(2) * 225 + 250);
        p.incQuestPoints(1);
    }
    ...
    public void onTalkToNpc(Player p, NPC n) {
        if (n.getID() == 470) {
            ...
            case 0:
                ...
                int option = showMenu(p, n, "i'm afraid i'm in a rush", "how can i help?");
                if (option == 0) {
    ...
    @Override
    public void onInvAction(InvItem i, final Player p) {
        if(i.getID() == 796) {
            ActiveTile tile = world.getTile(p.getLocation());
            if(tile.hasGameObject()) {
                p.getSender().sendMessage("You can't plant a tree here");
                return;
            }
            p.setBusy(true);
            removeItem(p, new InvItem(796));
            p.getSender().sendMessage("You open and plant the seed");
            world.registerGameObject(new GameObject(p.getLocation(), 490, 0, 0)); // 490 is a walkable tree
            world.getDelayedEventHandler().add(new DelayedEvent(null, 180000) {
                GameObject object = world.getTile(owner.getLocation()).getGameObject();
                @Override
                public void run() {
                    if (object != null) {
