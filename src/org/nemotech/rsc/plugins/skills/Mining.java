...
public final class Mining implements ObjectActionListener, ObjectActionExecutiveListener {
    ...
    public void onObjectAction(final GameObject object, String command,
        ...
        if (object.getID() == 269) {
            ...
            if (command.equalsIgnoreCase("mine")) {
                if(owner.getQuestStage(Plugin.WATCHTOWER) == 9) {
                    if (!hasItem(owner, getAxe(owner))) {
                        owner.message("You need a pickaxe to mine the rock");
                        return;
                    }
                    if(owner.getMaxStat(14) < 40) {
    ...
    public void handleMining(final GameObject object, Player owner, int click) {
        ...
        final ObjectMiningDef def = EntityManager.getObjectMiningDef(object.getID());
        ...
        if (def == null || def.getRespawnTime() < 1 || (def.getOreId() == 315 && owner.getQuestStage(FAMILY_CREST) < 6)) {
        ...
        if (owner.click == 1) {
            ...
            owner.message("This rock contains " + new InvItem(def.getOreId()).getDef().getName());
        ...
        owner.getSender().sendSound(SoundEffect.MINE);
        ...
        owner.setBatchEvent(new BatchEvent(owner, 2000, retrytimes) {
            ...
            public void action() {
                ...
                if (Formulae.getOre(def, owner.getCurStat(14), axeId) && mineLvl >= def.getReqLevel()) {
                    if (Util.random(0, 200) == 1) {
