...
import org.nemotech.rsc.plugins.listeners.action.*;
import org.nemotech.rsc.plugins.listeners.executive.*;
import org.nemotech.rsc.util.Formulae;

public class Thieving extends Plugin
    ...
    public void stallThieving(Player player, GameObject object, Stall stall) {
        ...
        sleep(800);
        if(player.getCurStat(17) < stall.getRequiredLevel()) {
            player.message("Your theiving ability is not high enough to thieve from stall");
            return;
        }

        NPC shopkeeper = getNearestNpc(player, stall.getOwnerID(), 8);
        NPC guard = null;
        if(stall.equals(Stall.BAKERS_STALL)) {
        ...
        double random = Util.random(1, 100);
        InvItem selectedLoot = null;
        for(LootItem loot : stall.lootTable) {
            if(loot.getChance() >= random) {
                selectedLoot = new InvItem(loot.getId(), loot.getAmount());
                break;
            }
        }
        if(selectedLoot == null) {
            selectedLoot = new InvItem(stall.lootTable.get(0).getId(), stall.lootTable.get(0).getAmount());
            return;
        }
        ...
        player.incExp(17, (int) stall.getXp(), true);
        World.getWorld().replaceGameObject(object, new GameObject(object.getLocation(), 341, object.getDirection(), object.getType()));
    ...
    public void handleChestThieving(Player player, GameObject obj) {
        ...
        switch(obj.getID()) {
            ...
            case 337:
                ...
                teleLoc = Point.getLocation(614, 568);
                break;
            case 338:
                // paladin Chest
                req = 72;
                ...
                teleLoc = Point.getLocation(523, 606);
                break;
        }

        player.message("You search the chest for traps");
        sleep(1200);
        if(player.getCurStat(17) < req) {
        ...
        for(LootItem l : loot) {
            if(l.getChance() >= random) {
                player.getInventory().add(new InvItem(l.getId(), l.getAmount()));
            }
        }
        player.incExp(17, (int) xp, true);
    ...
    public void doPickpocket(final Player player, final NPC npc, final Pickpocket pickpocket) {
        player.face(npc);
        if(player.getCurStat(17) < pickpocket.getRequiredLevel()) {
        ...
        player.message("You attempt to pick the " + npc.getDef().getName() + "'s pocket");
        player.setBatchEvent(new BatchEvent(player, 1300, Formulae.getRepeatTimes(player, THIEVING)) {
            @Override
            public void action() {

                owner.setBusyTimer(1300);
                npc.setBusyTimer(1300 * 2);
                if(npc.inCombat()) {
                    interrupt();
                    return;
                }
                boolean succeededPickpocket = succeedThieving(owner, pickpocket.getRequiredLevel());
                if(succeededPickpocket) {
                    if(owner.getFatigue() >= 7500) {
                        owner.message("@gre@You are too tired to gain experience, get some rest");
                    }

                    owner.incExp(17, (int) pickpocket.getXp(), true);
                    InvItem selectedLoot = null;
                    int total = 0;
                    ...
                    for(LootItem loot : lootTable) {
                        if(loot.getChance() >= 100.0) {
                            owner.getInventory().add(new InvItem(loot.getId(), loot.getAmount()));
                            continue;
                        }
                        if(hit >= total && hit < (total + loot.getChance())) {
                            if(loot.getId() == -1) {
                                owner.message("You find nothing to steal");
                                return;
                            }
                            selectedLoot = (new InvItem(loot.getId(), loot.getAmount()));
                            break;
                    ...
                    owner.message("You pick the " + npc.getDef().getName() + "'s pocket");
                    if(selectedLoot != null) {
                        owner.getInventory().add(selectedLoot);
                    }
                } else {
                    owner.face(npc);
                    ...
                    npc.setBusyTimer(0);
                    setDelay(650);
                    owner.message("You fail to pick the " + npc.getDef().getName() + "'s pocket");
                    interrupt();
                    npcTalk(owner, npc, pickpocket.shoutMessage);
                    npc.startCombat(owner);
                }
            }
        });
    }
    ...
    private boolean canBeSeen(int fromX, int fromY, int targetX, int targetY) {
        ...
        while(!stop) {
            ...
            if((World.getWorld().getTileValue(fromX, fromY).objectValue & 64) != 0) {
                stop = true;
                return false;
            }
    ...
    public static boolean succeedPickLockThieving(Player player, int req_level) {
        int level_diff = player.getCurStat(17) - req_level;

        int percent = Util.random(1, 100);
        if(level_diff < 0) {
            level_diff = 0;
        }
        ...
        if(level_diff > 40) {
            ...
            level_diff = (int) (player.getCurStat(17) * (double) 0.2D) + 50;
        }
    ...
    /**
     * Nature rune chest door id (94) [3.11.2013 22:02:55] Kevin: 10gp chest door id (93) [3.11.2013 22:05:47] Kevin: house where u can find nature chest + 10gp chest door id (94) [3.11.2013 22:07:17]
     * Kevin: Chaos druid tower door id (96) [3.11.2013 22:11:49] Kevin: yanille druid door id(162) [3.11.2013 22:15:08] Kevin: axe huts door id (100) [3.11.2013 22:17:28] Kevin: pirate hut door id
     * (99) [3.11.2013 22:22:29] Kevin:paladin 2nd floor door id (97) [3.11.2013 22:23:46] Kevin: the paladin chest give all the items it says it gie all at once [3.11.2013 22:23:58] Kevin: when you
