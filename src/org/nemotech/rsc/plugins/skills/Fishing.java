...
import org.nemotech.rsc.external.definition.extra.ObjectFishDef;
import org.nemotech.rsc.external.definition.extra.ObjectFishingDef;
...
public class Fishing implements ObjectActionListener, ObjectActionExecutiveListener {
    ...
    private void handleFishing(final GameObject object, Player owner, final int click, final String command) {
        final ObjectFishingDef def = EntityManager.getObjectFishingDef(object.getID(), click);
        ...
        if (owner.getFatigue() >= 7500) {
            owner.message("@gre@You are too tired to catch this fish");
            return;
        }
        if (owner.getCurStat(10) < def.getReqLevel()) {
            owner.message("You need at least level " + def.getReqLevel() + " "
                    + fishingRequirementString(object, command) + " "
                    + (def.getFishDefs().length > 1 ? "these fish"
                            : EntityManager.getItem(def.getFishDefs()[0].getId()).getName().toLowerCase()
                            .substring(4) + "s"));
            return;
        }
        final int netId = def.getNetId();
        if (owner.getInventory().countId(netId) <= 0) {
            owner.message("You need a "
                            + EntityManager
                            .getItem(
                                    netId)
                            .getName().toLowerCase()
        ...
        if (baitId >= 0) {
            if (owner.getInventory().countId(baitId) <= 0) {
                owner.message("You don't have any " + EntityManager.getItem(baitId).getName().toLowerCase() + " left");
                return;
            }
        }
        owner.message("You attempt to catch " + tryToCatchFishString(def));
        showBubble(owner, new InvItem(netId));
        owner.setBatchEvent(new BatchEvent(owner, 1900, Formulae.getRepeatTimes(owner, 10)) {
            ...
            public void action() {
                ...
                if (baitId >= 0) {
                    if (owner.getInventory().countId(baitId) <= 0) {
                        owner.message("You don't have any " + EntityManager.getItem(baitId).getName().toLowerCase()
                                + " left");
                        return;
                    }
                }
                ...
                owner.getSender().sendSound(SoundEffect.FISH);
                ObjectFishDef fishDef = getFish(def, owner.getCurStat(10), click);
                if (fishDef != null) {
                    if (baitId >= 0) {
                    ...
                    if(netId == 548) {
                        if(Util.random(0, 200) == 100) {
                            owner.message("You catch a casket");
                            owner.incExp(10, fishDef.getExp(), true);
                            addItem(owner, 549, 1);
                        }
                        InvItem fish = new InvItem(fishDef.getId());
                        owner.getInventory().add(fish);
                        owner.message("You catch " + (fish.getID() == 17 || fish.getID() == 622 || fish.getID() == 16 ? "some" : fish.getID() == 793 ? "an" : "a") + " "
                                + fish.getDef().getName().toLowerCase().replace("raw ", "").replace("leather ", "") + (fish.getID() == 793 ? " shell" : ""));
                        owner.incExp(10, fishDef.getExp(), true);
                    } else {
                        InvItem fish = new InvItem(fishDef.getId());
                        owner.getInventory().add(fish);
                        owner.message("You catch " + (netId == 376 ? "some" : "a") + " "
                                + fish.getDef().getName().toLowerCase().replace("raw ", "") + (fish.getID() == 349 ? "s" : "")
                                + (fish.getID() == 545 ? "!" : ""));
                        owner.incExp(10, fishDef.getExp(), true);
                    }
                } else {
                    owner.message("You fail to catch anything");
    ...
    public static ObjectFishDef getFish(ObjectFishingDef objectFishDef, int fishingLevel, int click) {
        ArrayList<ObjectFishDef> fish = new ArrayList<>();
        ...
        ObjectFishDef thisFish = fish.get(Util.random(0, fish.size() - 1));
        int levelDiff = fishingLevel - thisFish.getReqLevel();
        if (levelDiff < 0) {
            return null;
        }
        return Util.percentChance(offsetToPercent(levelDiff)) ? thisFish : null;
    }
